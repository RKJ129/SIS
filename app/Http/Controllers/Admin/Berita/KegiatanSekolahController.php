...
class KegiatanSekolahController extends Controller
{
    public function index() {
        $data = Berita::where('kategori', 'kegiatan sekolah')->get();
        return view('admin.berita.kegiatanSekolah.index', compact('data'));
    }

    public function store(BeritaRequest $request) {
        $imgName = time() . '_' . $request->file('img')->getClientOriginalName();
        $request->file('img')->move('img/berita/kegiatan-sekolah', $imgName);
    ...
    public function update(BeritaRequest $request, $id) {
        ...
        File::delete('img/berita/kegiatan-sekolah/' . $data->img);
